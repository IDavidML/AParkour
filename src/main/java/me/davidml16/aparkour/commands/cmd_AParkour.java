...
public class cmd_AParkour implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!(sender instanceof Player)) {
			sender.sendMessage("§cThe commands only can be use by players!");
			return true;
		}
	...
	private void sendParkourList(Player p) {
		p.sendMessage("");
		p.sendMessage("  §a§lParkours availables:");
		p.sendMessage("");
		
		if(Main.getInstance().getParkourHandler().getParkours().size() == 0) {
			p.sendMessage("    §7- §c§lAny parkour created!");
		} else {
			for (ParkourData parkour : Main.getInstance().getParkourHandler().getParkours().values()) {
				p.sendMessage("    §7- §e§lID: §a§l" + parkour.getId() + " §7§l| §e§lName: §a§l" + parkour.getName());
			}
		}
	...
	public void sendCommandHelp(Player p) {
		p.sendMessage("");
		p.sendMessage("§7 - §a/aparkour stats");
		p.sendMessage("§7 - §a/aparkour list");
		p.sendMessage("§7 - §a/aparkour top");
		p.sendMessage("");
		if (Main.getInstance().getPlayerDataHandler().playerHasPermission(p)) {
			p.sendMessage("§7 - §a/aparkour create [id] [name]");
			p.sendMessage("§7 - §a/aparkour remove [id]");
			p.sendMessage("§7 - §a/aparkour set [id] [name,spawn,start,end,stats,top]");
			p.sendMessage("§7 - §a/aparkour reload");
			p.sendMessage("");
		}
	}
